...
impl<Ctx: super::Context<'static>> PakFilesystem<'static, Ctx> {
    fn make_attribute(ino: INodeNo, kind: &EntryKind::<Ctx::Fk>) -> fuser::FileAttr {
...
impl<Ctx: super::Context<'static>> Filesystem for PakFilesystem<'static, Ctx> {
    ...
    fn read(
        ...
        if !self.cache.read().contains_key(&ino) {
            ...
            let EntryKind::File { paths, parser_kind, .. } = kind else {return reply.error(Errno::ENOENT)};
            
            if let Ok(content) = self.parse_file(parser_kind, paths) {
