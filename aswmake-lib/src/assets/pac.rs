...
declare_asset! {
    ...
    compile_glob: Some("**/Content/Chara/**/Data/**/COL_*.pac"),
    ...
    fn parse_fn(asset: &Asset, reader: &PakReader, _: Option<&dyn std::any::Any>) -> AResult<Vec<u8>> {
        debug_dbg!("Parsing pac with rel_path {} for {:?}", asset.rel_path_noext.display(), asset.target_game);
    ...
    fn query_size_fn(asset: &Asset, reader: &PakReader, _: Option<&dyn std::any::Any>) -> AResult<Option<u64>>{
        debug_dbg!("Querying pac size with rel_path {} for {:?}", asset.rel_path_noext.display(), asset.target_game);
